...
- `npm run og` — regenerate stale writing share cards in `public/og/` (`-- --force` for all); commit the PNGs. Pieces with `ogManual: true` are skipped
...
- Share cards: `public/og/<slug>.png`, generated by `npm run og` and committed
  (freshness tracked in `public/og/manifest.json`; `npm run check` warns when
  stale). Card text comes from `shortTitle ?? title` and
  `ogSubtitle ?? shortSubtitle ?? subtitle` — never `summary`. `ogManual: true`
  keeps a hand-made card. Design/version notes live in
  `scripts/generate-og-writing.mjs` and `scripts/og-hash.mjs` — the byline is
  drawn as glyph outlines because librsvg can't render Copperplate30.otf.
